...
const  prod = process.env.NODE_ENV != 'development' ? true : false;
const  isBuild=process.env.NODE_ENV==='production'?true : false;

let hostName, devPort;

//可以设置自己电脑IP或者其他
hostName = "127.0.0.1";
// hostName = "192.168.1.164";//陈博先电脑ip
devPort = "8018";
...
entryObj=Object.assign(entryObj,{
    vendors:['vue',resolve('./src/APPcommon/modules/common.js')]
})
...
module.exports = {
    ...
    plugins: [
        ...
        new webpack.optimize.CommonsChunkPlugin({
            ...
            minChunks: Math.max(pageNameList.length-1,2),//至少三个模块共有部分，才会进行提取
        ...
        new OpenBrowserPlugin({
            url: 'http://' + hostName + ':' + devPort + '/webpack-dev-server',//测试页面选择test.html，可以自己更改
        }),
        //热替换
...
function getHtmlPlugin(name) {
    ...
    return (new HtmlWebpackPlugin({
        ...
        template: './src/APPcommon/view/index.ejs',
        inject: 'body',
        chunks: ['vendors', name],
        minify: {
        ...
        // hash:true,
        cache:true,
        agreement:isBuild?'https://':'http://t'
    })
    )
}